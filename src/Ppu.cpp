...
void enter_vblank() {
	...
	if (LIKELY(ppu_cycle_ != (ppu_read_2002_cycle_ + 1))) {
		status_.vblank = true;
	}
}
		...
		if (hpos_ < 1) {
			...
			case 338:
				...
				if (odd_frame_ && odd_skip_armed_) {
					// Odd-frame skip happens here; emulate dot 339's open-bus phase
					// so mapper/address-side behavior stays stable.
					open_tile_index();
					++hpos_;
				}
				break;
...
void clock_ppu(const scanline_vblank &target) {
	...
	if UNLIKELY (vpos_ == 242) {
		...
		case 1:
			...
			break;
		case 3:
			if UNLIKELY (ppu_control_.nmi_on_vblank && status_.vblank) {
				cpu::nmi();
			}
			break;
		}
