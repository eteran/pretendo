...
emulate all the undocumented instructions. Select between these by
holding the following buttons on controller #1 when starting the test:
...
timing, instructions reference address $0080 and X and Y are set to 0.
To test page crossing timing, X and Y are set to $FE. This causes a page
crossing for relevant instructions as $0080+$FE=$0017E, which is on a
different page. Not all instructions add an extra clock when a page is
crossed, so this test reveals errors that go both ways.
...
at $0202 (for RTI) or $0203 (for RTS, since it adds one to the return
address). BRK is handled by setting the IRQ vector to $0202, the next
instruction in the loop after BRK. The trickiness of these special cases
might reveal non-timing problems in an emulator.
